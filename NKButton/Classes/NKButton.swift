...
open class NKButton: UIButton {
    ...
    fileprivate var imgBgColorDict      : [String : UIColor] = [:]
    ...
    */
    ...
    override open func draw(_ rect: CGRect) {
        ...
        var currentState     = isHovering ? [state, .hovered] : state
        ...
        let imageBackgroundColor = imageBackgroundColor(for: currentState) ?? imageBackgroundColor(for: state) ?? imageBackgroundColor(for: .normal)
        let imageBorderColor     = imageBorderColor(for: currentState)
        let imageBorderSize      = imageBorderSize(for: currentState)

        imageView?.backgroundColor   = imageBackgroundColor
        imageView?.layer.borderColor = imageBorderColor?.cgColor
        imageView?.layer.borderWidth = imageBorderSize
...
//            #endif
    ...
    open func setImageBackgroundColor(_ color: UIColor?, for state: UIControl.State) {
        let key = imageBackgroundColorKey(for: state)
        imgBgColorDict[key] = color
        setNeedsDisplay()
    }
    ...
    open func setImageBorderColor(_ color: UIColor?, for state: UIControl.State) {
        let key = imgBorderColorKey(for: state)
        imgBorderColorDict[key] = color
        setNeedsDisplay()
    }
    ...
    open func setImageBorderSize(_ value: CGFloat?, for state: UIControl.State) {
        let key = imgBorderSizeKey(for: state)
        imgBorderSizeDict[key] = value
        setNeedsDisplay()
    }
    ...
    open func imageBackgroundColor(for state: UIControl.State) -> UIColor? {
        let key = imageBackgroundColorKey(for: state)
        return imgBgColorDict[key]
    }
    ...
    open func imageBorderColor(for state: UIControl.State) -> UIColor? {
        let key = imgBorderColorKey(for: state)
        var result = imgBorderColorDict[key]
        return result
    }
    ...
    open func imageBorderSize(for state: UIControl.State) -> CGFloat {
        let key = imgBorderSizeKey(for: state)
        return imgBorderSizeDict[key] ?? 0
    }
    ...
    fileprivate func imageBackgroundColorKey(for state: UIControl.State) -> String {
        return "imgbg\(state.rawValue)"
    }
    ...
    fileprivate func imgBorderColorKey(for state: UIControl.State) -> String {
        return "imgbr\(state.rawValue)"
    }
    ...
    fileprivate func imgBorderSizeKey(for state: UIControl.State) -> String {
        return "imgbs\(state.rawValue)"
    }
